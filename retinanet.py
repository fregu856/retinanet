...
#
#         self.anchors_per_cell = 9 # (num anchors per grid point)
#         self.num_classes = 2 # (background, car)
#
#         self.fpn = FPN18() # NOTE! (FPN18, FPN34, FPN50, FPN101 or FPN152)
#
#         self.class_conv1 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
...
#         self.fpn = FPN18() # NOTE! (FPN18, FPN34, FPN50, FPN101 or FPN152)
#
#         self.var_conv1 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.var_conv2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.var_conv3 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.var_conv4 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.var_conv5 = nn.Conv2d(256, 4*self.anchors_per_cell, kernel_size=3, stride=1, padding=1)
#
#         self.regr_conv1 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.regr_conv2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
#         self.regr_conv3 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
...
#             outputs_regr.append(out_regr)
#
#             out_var = F.relu(self.var_conv1(feature_map)) # (shape: (batch_size, 256, h/y, w/y))
#             out_var = F.relu(self.var_conv2(out_var)) # (shape: (batch_size, 256, h/y, w/y))
#             out_var = F.relu(self.var_conv3(out_var)) # (shape: (batch_size, 256, h/y, w/y))
#             out_var = F.relu(self.var_conv4(out_var)) # (shape: (batch_size, 256, h/y, w/y))
#             out_var = self.var_conv5(out_var) # (shape: (batch_size, 4*self.anchors_per_cell, h/y, w/y))
#             # # NOTE! NOTE! or is it this that messes things up?
#             # out_regr = out_regr.view(-1, batch_size, 4) # (shape: ((h/y)*(w/y)*self.anchors_per_cell, batch_size, 4))
#             # out_regr = out_regr.permute(1, 0, 2) # (shape: (batch_size, (h/y)*(w/y)*self.anchors_per_cell, 4)) (resid_x, resid_y, resid_w, resid_h)
#             out_var = out_var.permute(0, 2, 3, 1) # (shape: (batch_size, h/y, w/y, 4*self.anchors_per_cell))
#             out_var = out_var.contiguous().view(out_var.size(0), -1, 4) # (shape: (batch_size, (h/y)*(w/y)*self.anchors_per_cell, 4))
#             outputs_var.append(out_var)
#
#         outputs_regr = torch.cat(outputs_regr, 1) # (shape: (batch_size, num_anchors, 4)) (num_anchors: total number of anchors in the image) (resid_x, resid_y, resid_w, resid_h)
#         outputs_var = torch.cat(outputs_var, 1) # (shape: (batch_size, num_anchors, 4)) (num_anchors: total number of anchors in the image) (var_x, var_y, var_w, var_h)
#
#         return (outputs_regr, outputs_var)
#
#     def create_model_dirs(self):
#         self.logs_dir = self.project_dir + "/training_logs"
#         self.model_dir = self.logs_dir + "/model_%s" % self.model_id
