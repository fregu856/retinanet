...
from datasets import DatasetMoreAugmentation2, DatasetEval, DatasetSynscapesAugmentation, DatasetSynscapesEval # (this needs to be imported before torch, because cv2 needs to be imported before torch for some reason)
from retinanet import RetinaNet
...
#     network.train() # (set in training mode, this affects BatchNorm and dropout)
#     batch_losses = []
#     for step, (imgs, labels_regr) in enumerate(train_loader):
#         #current_time = time.time()
#
#         imgs = Variable(imgs).cuda() # (shape: (batch_size, 3, img_heigth, img_width))
#         labels_regr = Variable(labels_regr).cuda() # (shape: (batch_size, num_anchors, 4)) (x_resid, y_resid, w_resid, h_resid)
...
#         outputs = network(imgs)
#         outputs_regr = outputs[0] # (shape: (batch_size, num_anchors, 4)) (x_resid, y_resid, w_resid, h_resid)
#         outputs_var = outputs[1] # (shape: (batch_size, num_anchors, 4)) (x_var, y_var, w_var, h_var) (log(sigma^2))
#
#         ########################################################################
#         # compute the loss:
#         ########################################################################
#         loss = torch.mean(torch.exp(-outputs_var)*torch.pow(labels_regr - outputs_regr, 2) + outputs_var)
#
#         if step == 0:
#             print ("outputs_regr.data.cpu().numpy():")
#             print (outputs_regr.data.cpu().numpy())
...
#             print (labels_regr.data.cpu().numpy())
#             print (labels_regr.data.cpu().numpy().shape)
#             print ("torch.exp(outputs_var).data.cpu().numpy():")
#             print (torch.exp(outputs_var).data.cpu().numpy())
#             print (torch.exp(outputs_var).data.cpu().numpy().shape)
#
#         loss_value = loss.data.cpu().numpy()
#         batch_losses.append(loss_value)
#
...
#     network.eval() # (set in evaluation mode, this affects BatchNorm and dropout)
#     batch_losses = []
#     for step, (imgs, labels_regr, img_ids) in enumerate(val_loader):
#         with torch.no_grad(): # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
#             imgs = Variable(imgs).cuda() # (shape: (batch_size, 3, img_heigth, img_width))
#             labels_regr = Variable(labels_regr).cuda() # (shape: (batch_size, num_anchors, 4)) (x_resid, y_resid, w_resid, h_resid)
#
